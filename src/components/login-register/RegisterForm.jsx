...
export default function RegisterForm({ setShowFormStatus }) {
    ...
    const registeValidation = (event) => {
        ...
        if (name === "firstName") {
            setErrors({ ...errors, fristNameErrors: value.length === 0 ? "يجب ادخال الاسم الاول"
             :nameRegx.test(value)
             ? ""
             : "يجب ادخال ثلاثة احرف بحد ادني",});
        }
        if (name === "lastName") {
            setErrors({ ...errors, lastNameErrors: value.length === 0 ? "يجب ادخال الاسم الاخير"
            :nameRegx.test(value)
            ? ""
            : "يجب ادخال ثلاثة احرف بحد ادني",});
        }
        if (name === "email") {
            setErrors({ ...errors, emailErrors: value.length === 0 ? "يجب ادخال البريد الاليكتروني"
            :emailRegx.test(value)
            ? ""
            : "يجب ادخال بريد اليكتروني صحيح", });
        }
        if (name === "password") {
            setErrors({ ...errors, passwordErrors: value.length === 0 ?  "يجب ادخال الاسم الاول"
        ...
        if (name === "phoneNumber") {
            setErrors({ ...errors, phoneNumberErrors: value.length === 0 ? "phoneNumber" : "" });
        }
        if (name === "role") {
        ...
        if (registeUser.role === 'driver'){
            if (name === "carType") {
                setErrors({ ...errors, carTypeErrors: value.length === 0 ? "carTypeErrors" : "" });
            }
            if (name === "plateNumber") {
                setErrors({ ...errors, plateNumberErrors: value.length === 0 ? "plateNumberErrors" : "" });
            }
        }
        if (registeUser.role === 'renter'){
            ...
            if (name === "region") {
                setErrors({ ...errors, regionErrors: value.length === 0 ? "regionErrors" : "" });
            }
        }
